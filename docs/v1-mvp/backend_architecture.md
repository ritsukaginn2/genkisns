...
V1 主流程不依赖后端。V1.6 已在 `services/llm-proxy` 实现审核与 LLM 后端，用来做用户/安装实例审核、内容安全审核和服务端 LLM 调用；该后端不接管 V1 的笔记主数据。
...
| V1.6 后端 | 可选接入审核与 LLM 后端 | 只负责审核、风控、队列和服务端 LLM 调用，不保存 V1 笔记主数据 |
...
V1 不依赖云端 LLM。`InteractionService` 在本机根据笔记内容、媒体类型、图片数量、默认用户资料和 AI 好友人设生成点赞数与评论。生成失败时使用最小备用模板，不阻塞发帖。

V1.6 接入后，真实 LLM 生成应走后端审核与代理调用；App 仍先保留本地模板互动，后端成功后再用真实结果更新本地 SQLite。
...
Phase 4 起，Flutter 原生运行与数据层验收必须通过 `flutter run` 在模拟器或真实设备中完成。`board.html` 和 `build/web` 只作为设计归档或开发调试，不作为实现验收入口。
...
7. 建立系统相机拍照/拍视频、App 内相册选择、已选回显、本地文件复制和媒体引用模型。（已完成）
