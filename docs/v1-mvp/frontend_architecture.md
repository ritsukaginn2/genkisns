  ...
  lib/
    ...
    data/
      ...
      services/
        ...
        image_picker_service.dart ← 拍照、拍视频、App 内相册选择、文件复制和本地引用生成
      stores/
...
- 发布页点击添加媒体后，先出现媒体来源选择层：拍照、拍视频、相册、取消。
- 拍照一次添加 1 张图片。
- 拍视频一次添加 1 个视频。
...
首页空状态
  -> 发布笔记
  -> 输入文字或添加媒体：拍照 / 拍视频 / 相册图片多选 / 相册视频单选 / 已选回显
  -> 发布成功回首页
  -> 打开笔记详情
...
- 拍照、拍视频和相册媒体会复制到 App 文档目录，数据库保存 `localRef`；视频额外保存 `thumbnailRef` 和 `durationMillis`。
